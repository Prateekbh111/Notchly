...
- **Menu-bar toggle** — enable/disable the island from the menu bar without quitting the app.
...
1. Unzip `DynamicIsland-YYYY-MM-DD.zip` to `/Applications`.
2. First launch: right-click `Dynamic island.app` → **Open** to bypass Gatekeeper (binary is ad-hoc signed only).
...
```bash
git clone <repo-url> dynamic-island
cd "dynamic-island"
xcodebuild \
...
- **Volume / brightness keys** are routed through the notch HUD when the app has Accessibility permission. Without it, the system OSD shows as usual.
...
```
Dynamic island/                 macOS app target
├── App/
